...
#define LOGI(...) \
  ((void)__android_log_print(ANDROID_LOG_INFO, "ijkplayer_jni", __VA_ARGS__))
...
AndroidHalFourccDescriptor *native_window_get_desc(int fourcc_or_hal)
{
    for (int i = 0; i < NELEM(g_hal_fcc_map); ++i) {
        ...
        LOGI("i :%d, fourcc_or_hal:%d,fcc_or_hal:%d",i,fourcc_or_hal,desc->fcc_or_hal);

...
struct fields_t {
    ...
    //jmethodID eraseColorMethod;
    jmethodID reconfigureMethod;
...
/*  cyj begin */
/*static jobject wseemann_media_FFmpegMediaMetadataRetriever_getFrameAtTime2(JNIEnv *env, jobject thiz, jlong timeUs, jint option)
{
   //__android_log_write(ANDROID_LOG_INFO, LOG_TAG, "getFrameAtTime");


   return wseemann_media_FFmpegMediaMetadataRetriever_getScaledFrameAtTime2(env, thiz, timeUs, option, -1, -1);
}*/

static void wseemann_media_FFmpegMediaMetadataRetriever_getScaledFrameAtTime2(JNIEnv *env, jobject thiz, jlong timeUs, jint option, jobject jBitmap)
{
    ALOGI("getScaledFrameAtTime2");
    MediaMetadataRetriever* retriever = getRetriever(env, thiz);
    if (retriever == 0) {
        jniThrowException(env, "java/lang/IllegalStateException", "No retriever available");
        //return NULL;
    ...
    int h = info.height;
    int fmt = info.format;
    if(w <= 10 || h <= 10 || fmt != ANDROID_BITMAP_FORMAT_RGB_565){
        ALOGE("getScaledFrameAtTime2 Illegal bitmap parameter w=%d, h=%d, fmt=%d", w, h, fmt);
        return;
    }
    ...
    if (retriever->getScaledFrameAtTime2(timeUs, option, &videoFrame, w, h) != 0) {
       ALOGE("getScaledFrameAtTime2: failed!!!");
        //return NULL;
        return;
    }

    ALOGI("Dimension = %dx%d and bytes = %d",
            videoFrame.width,
            videoFrame.height,
            videoFrame.linesize[0]);

            
     const char *filename = "/sdcard/two.rgb";
    FILE *picture = fopen(filename, "wb");        
    fwrite(videoFrame.data[0], videoFrame.linesize[0] * videoFrame.height, 1, picture);
    fclose(picture);
#if 0
    jobject jBitmap = NULL;
    jintArray array = NULL;
    array = env->NewIntArray(videoFrame.width * videoFrame.height);
    if (!array) {  // OutOfMemoryError exception has already been thrown.
        ALOGE("NewIntArray failed !!");
        return NULL;
    } else {
        jint* bytes = env->GetIntArrayElements(array, NULL);
        if (bytes != NULL) {
            ALOGI("copy bytes len");
            //memcpy(bytes, videoFrame.data[0], videoFrame.linesize[0] * videoFrame.height);
            env->ReleaseIntArrayElements(array, bytes, 0);

            jint colorCfg = 3;   /*3:565 5:8888*/
            ...
            jBitmap = env->CallStaticObjectMethod(
                                    fields.bitmapClazz,
             ...
             if (jBitmap == NULL) {
                ...
                ALOGE("getScaledFrameAtTime2: create Bitmap failed!");
                return NULL;
            }
    ...
    //AndroidBitmapInfo info;
    AndroidBitmap_getInfo(env, jBitmap, &info);
    ALOGI("bitmap Dimension = %ux%u stride=%u fmt = %d",
            info.width,
            info.height,
            info.stride,
            info.format);
    //jint color = 0x7e0;
    // (jobject) env->CallObjectMethod(jBitmap, fields.eraseColorMethod, color); 
    if((result = AndroidBitmap_lockPixels(env, jBitmap, &dataPtr)) < 0){
       ALOGE("getScaledFrameAtTime2: AndroidBitmap_lockPixels failed! result=%d", result);
    }
    if(dataPtr) {
        memcpy(dataPtr, videoFrame.data[0], info.width * info.height * sizeof(uint16_t));
        ALOGI("getScaledFrameAtTime2 ok");
    }else{
        ALOGE("!!!!!dataPtr =%p", dataPtr);
    }
...
static void wseemann_media_FFmpegMediaMetadataRetriever_native_init(JNIEnv *env, jobject thiz)
{
    ...
    fields.reconfigureMethod = 
              env->GetMethodID(fields.bitmapClazz, "reconfigure",
                    "(IILandroid/graphics/Bitmap$Config;)V");
    if (fields.reconfigureMethod == NULL) {
        ALOGE("get reconfigureMethod failed !!!");
        return;
    }
...
static JNINativeMethod nativeMethods[] = {
    ...
    /* cyj begin */
    //{"_getFrameAtTime2", "(JI)Landroid/graphics/Bitmap;", (void *)wseemann_media_FFmpegMediaMetadataRetriever_getFrameAtTime2},
    {"_getScaledFrameAtTime2", "(JILandroid/graphics/Bitmap;)V", (void *)wseemann_media_FFmpegMediaMetadataRetriever_getScaledFrameAtTime2},
    /* cyj end */
